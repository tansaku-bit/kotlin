...
fun IrType.isNullable(): Boolean =
    when (this) {
        is IrSimpleType -> when (val classifier = classifier) {
            is IrClassSymbol -> nullability == SimpleTypeNullability.NULLABLE
            is IrTypeParameterSymbol -> when (nullability) {
                SimpleTypeNullability.NULLABLE -> true
                SimpleTypeNullability.NOT_SPECIFIED -> classifier.owner.superTypes.any(IrType::isNullable) // TODO, why any, shouldn't be all?
