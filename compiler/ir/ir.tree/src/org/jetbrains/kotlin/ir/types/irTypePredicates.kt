...
fun IrType.isPrimitiveType(nullable: Boolean = false): Boolean =
    nullable == this.isNullable() && getPrimitiveType() != null
...
fun IrType.isUnsignedType(nullable: Boolean = false): Boolean =
    nullable == this.isNullable() && getUnsignedType() != null
...
fun IrType.isMarkedNullable() = (this as? IrSimpleType)?.nullability == SimpleTypeNullability.NULLABLE
...
fun IrType.isClassType(fqName: FqNameUnsafe, nullable: Boolean): Boolean {
    if (this !is IrSimpleType) return false
    if (this.isNullable() != nullable) return false
