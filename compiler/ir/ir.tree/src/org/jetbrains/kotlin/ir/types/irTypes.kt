...
private fun IrSimpleType.withNullability(newNullability: Boolean): IrSimpleType {
    val requiredNullability = if (newNullability) SimpleTypeNullability.NULLABLE else SimpleTypeNullability.DEFINITELY_NOT_NULL
...
fun IrType.mergeNullability(nullability: SimpleTypeNullability) = when (nullability) {
    SimpleTypeNullability.NULLABLE -> makeNullable()
...
fun IrType.mergeNullability(other: IrType) = when (other) {
    is IrSimpleType -> mergeNullability(other.nullability)
    else -> this
}
...
fun IrType.toKotlinType(): KotlinType {
    ...
    return when (this) {
        is IrSimpleType -> makeKotlinType(classifier, arguments, nullability == SimpleTypeNullability.NULLABLE)
        else -> TODO(toString())
    }
}
